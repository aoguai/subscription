    ...
    rules: [
      {
        ...
        matches: `${COMMON_PREFIX}[(text.length<=6&&(text~="跳[\\\\s]+过"||text~="跳[\\\\s]+過"||text~="(?is).*skip.*"))||id~="(?is).*tt_splash_skip_btn"||vid~="(?is).*skip.*"||(vid~="(?is).*count.*"&&vid~="(?is).*down.*"&&!(vid~="(?is).*load.*")&&!(vid~="(?is).*hour.*")&&!(vid~="(?is).*minute.*")&&!(vid~="(?is).*timing.*")&&!(vid~="(?is).*add.*")&&!(vid~="(?is).*ead.*"))||desc~="跳[\\\\s]+过"||desc~="跳[\\\\s]+過"||desc~="(?is).*skip.*"][!(text~="([01]?[0-9]|2[0-3])[:：][0-5][0-9]")][!(desc~="([01]?[0-9]|2[0-3])[:：][0-5][0-9]")]`,
        excludeSnapshotUrls: [
          // 避免误触
