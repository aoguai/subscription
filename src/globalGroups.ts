    ...
    rules: [
      {
        key: 0,
        matches: [
          `${COMMON_PREFIX}[(text*="将使用"||text*="是否允许"||text*="申请"||text*="需要"||text*="开启"||text*="打开"||text*="获取"||text*="Allow")&&(text*="权限"||text~="(?is).*Permissions.*"||((text*="剪切板"||text*="剪贴板"||text*="粘贴")&&(text*="内容"||text*="信息")))&&(text!*="定位"&&text!*="位置"&&!(text~="(?is).*location.*")&&text!*="通知"&&text!*="成功"&&text!*="失败"&&(text!*="已"||text*="已知"))]`,
      ...
      },
      {
        key: 1,
        matches: [
          `${COMMON_PREFIX}[(desc*="将使用"||desc*="是否允许"||desc*="申请"||desc*="需要"||desc*="开启"||desc*="打开"||desc*="获取"||desc*="Allow")&&(desc*="权限"||desc~="(?is).*Permissions.*"||((desc*="剪切板"||desc*="剪贴板"||desc*="粘贴")&&(desc*="内容"||desc*="信息")))&&(desc!*="定位"&&desc!*="位置"&&!(desc~="(?is).*location.*")&&desc!*="通知"&&desc!*="成功"&&desc!*="失败"&&(desc!*="已"||desc*="已知"))]`,
